...
xn = np.concatenate([train_text, train_parents, train_siblings, train_key], axis=1)
...
for epoch in range(N_EPOCHS):
    ...
    for i, (inputs, labels) in enumerate(train_loader):
        ...
        loss.backward()
        optimizer.step()
    if not os.path.exists(f'type10/checkpoints/{level}/{aim}'):
        os.mkdir(f'type10/checkpoints/{level}/{aim}')
    torch.save(model.state_dict(), f'type10/checkpoints/{level}/{aim}/epoch_{epoch}.pt')
    ...
    with open(f'type10/logfiles/{level}/{aim}.txt','a') as log:
