...
xn = np.concatenate([train_text, train_parents, train_siblings], axis=1)
...
for epoch in range(N_EPOCHS):
    ...
    for i, (inputs, labels) in enumerate(train_loader):
        ...
        loss.backward()
        optimizer.step()
    if not os.path.exists(f'type8/checkpoints/{level}/{aim}'):
        os.mkdir(f'type8/checkpoints/{level}/{aim}')
    torch.save(model.state_dict(), f'type8/checkpoints/{level}/{aim}/epoch_{epoch}.pt')
    ...
    with open(f'type8/logfiles/{level}/{aim}.txt','a') as log:
