...
modelP_level1 = PrivacyTagger.load_from_checkpoint(f'type5/cross_validation/checkpoints/segment/fold{fold}/1/epoch_9.ckpt', n_classes=14, logfile_path='').to(device)
#modelP = AutoModel.from_pretrained("mukund/privbert")
modelT_level1 = modelP_level1.to(device)
modelT_level1.eval()

modelP_level2_1 = PrivacyTagger.load_from_checkpoint(f'type5/cross_validation/checkpoints/segment/fold{fold}/2_1/epoch_9.ckpt', n_classes=2, logfile_path='').to(device)
#modelP = AutoModel.from_pretrained("mukund/privbert")
modelT_level2_1 = modelP_level2_1.to(device)
modelT_level2_1.eval()

modelP_level2_2 = PrivacyTagger.load_from_checkpoint(f'type5/cross_validation/checkpoints/segment/fold{fold}/2_2/epoch_9.ckpt', n_classes=8, logfile_path='').to(device)
#modelP = AutoModel.from_pretrained("mukund/privbert")
modelT_level2_2 = modelP_level2_2.to(device)
modelT_level2_2.eval()


modelP_level2_3 = PrivacyTagger.load_from_checkpoint(f'type5/cross_validation/checkpoints/segment/fold{fold}/2_3/epoch_9.ckpt', n_classes=2, logfile_path='').to(device)
#modelP = AutoModel.from_pretrained("mukund/privbert")
modelT_level2_3 = modelP_level2_3.to(device)
modelT_level2_3.eval()


modelP_level2_4 = PrivacyTagger.load_from_checkpoint(f'type5/cross_validation/checkpoints/segment/fold{fold}/2_4/epoch_9.ckpt', n_classes=4, logfile_path='').to(device)
#modelP = AutoModel.from_pretrained("mukund/privbert")
modelT_level2_4 = modelP_level2_4.to(device)
modelT_level2_4.eval()


modelP_level2_5 = PrivacyTagger.load_from_checkpoint(f'type5/cross_validation/checkpoints/segment/fold{fold}/2_5/epoch_9.ckpt', n_classes=3, logfile_path='').to(device)
#modelP = AutoModel.from_pretrained("mukund/privbert")
modelT_level2_5 = modelP_level2_5.to(device)
modelT_level2_5.eval()


modelP_level2_6 = PrivacyTagger.load_from_checkpoint(f'type5/cross_validation/checkpoints/segment/fold{fold}/2_6/epoch_9.ckpt', n_classes=2, logfile_path='').to(device)
#modelP = AutoModel.from_pretrained("mukund/privbert")
...
for i in range(7):
    ...
    with torch.no_grad():
        ...
        for j, data in enumerate(test_loader):
            ...
            outputs = model(input_ids, attention_mask)[1].detach().cpu()
...
with open(f'type5/cross_validation/logfiles/segment/fold{fold}/results.txt','a') as log:
