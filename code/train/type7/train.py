...
for epoch in range(N_EPOCHS):
    ...
    for i, (inputs, labels) in enumerate(train_loader):
        ...
        loss.backward()
        optimizer.step()
    if not os.path.exists(f'type7/checkpoints/{level}/{aim}'):
        os.mkdir(f'type7/checkpoints/{level}/{aim}')
    torch.save(model.state_dict(), f'type7/checkpoints/{level}/{aim}/epoch_{epoch}.pt')
    ...
    with open(f'type7/logfiles/{level}/{aim}.txt','a') as log:
