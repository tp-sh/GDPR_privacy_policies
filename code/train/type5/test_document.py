...
def text_my(sentence):
    text = cleanPunc(sentence)
    text = keepAlpha(text)
...
def term_identification(number):
    ...
    pFN = [0]*35
    try:
        tree = ET.parse('/data/data1/cyx/GoPPC-150/' + str(number) + '.xml')
    except:
    ...
    for child in root.iter():
        ...
        if child.tag == 'paragraph':
            ...
            if ti_text != '' and ti_text!=None:
                ...
                parent_text = text_my(parent_text)

                sibling_text = text_my(sibling_text)
                ...
                _, output = modelP_level1(inputs1['input_ids'], inputs1['attention_mask'])
                _, output2_1 = modelP_level2_1(inputs1['input_ids'], inputs1['attention_mask'])
                _, output2_2 = modelP_level2_2(inputs1['input_ids'], inputs1['attention_mask'])
                _, output2_3 = modelP_level2_3(inputs1['input_ids'], inputs1['attention_mask'])
                _, output2_4 = modelP_level2_4(inputs1['input_ids'], inputs1['attention_mask'])
                _, output2_5 = modelP_level2_5(inputs1['input_ids'], inputs1['attention_mask'])
                _, output2_6 = modelP_level2_6(inputs1['input_ids'], inputs1['attention_mask'])
                for t in range (0,14):
...
reslog.write('average F1 for level1: '+str(np.mean(pF1[:14]))+'\n')
