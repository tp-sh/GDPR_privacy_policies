...
parser.add_argument('--level', type=str, default='segment') #document
...
class PrivacyTagger(pl.LightningModule):
    def __init__(self, n_classes: int, steps_per_epoch=None, n_epochs=None):
        ...
        hidden_size = self.bert.config.hidden_size*3
        self.classifier = nn.Linear(hidden_size, n_classes)
        self.steps_per_epoch = steps_per_epoch
        self.n_epochs = n_epochs
        self.criterion = nn.BCELoss()
        
    def forward(self, input_ids, attention_mask, input_ids2, attention_mask2, input_ids3, attention_mask3,labels=None):
        output1 = self.bert(input_ids, attention_mask=attention_mask)
        output2 = self.bert(input_ids2, attention_mask=attention_mask2)
        output3 = self.bert(input_ids3, attention_mask=attention_mask3)
        output4 = torch.cat([output1.pooler_output, output2.pooler_output, output3.pooler_output], 1)
        #print(output4.shape)
        output = self.classifier(output4)
    ...
    def training_step(self,batch, batch_idx):
        ...
        labels = batch['labels']
        loss, outputs = self(input_ids, attention_mask, input_ids2, attention_mask2, input_ids3, attention_mask3, labels)
    ...
    def validation_epoch_end(self, outputs):
        ...
        with open(f'type6/logfiles/{level}/{aim}.txt','a') as logfile:
            logfile.write(mark+'\n')
...
class SaveCheckpointCallback(Callback):
    def on_epoch_end(self, trainer, pl_module):
        trainer.save_checkpoint(f"type6/checkpoints/{level}/{aim}/epoch_{trainer.current_epoch}.ckpt")
